        ...
        foreach ($tasks as $index => $task) {
            ...
            foreach ($arguments as $name => $argument) {
                ...
                if ($type === "string" && !is_string($value) && !(!$strictTypes && is_int($value))) {
                    ...
                    $type === "integer" &&
                    ((isset($spec["parameters"][$name]["min"]) && $value < $spec["parameters"][$name]["min"]) ||
                    ...
                    $errors[] = sprintf(
                        "%s.arguments.%s.value должен быть скалярным значением списка.",
                        $prefix,
                        $name,
                    );
                } elseif ($type === "list") {
            ...
            foreach ($spec["parameters"] ?? [] as $name => $parameter) {
                ...
                if (
                    ...
                    (!array_key_exists($name, $arguments) || $value === null || $value === "")
                ) {
