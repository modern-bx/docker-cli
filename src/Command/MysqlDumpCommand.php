...
final class MysqlDumpCommand extends AbstractCommand {
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        try {
            $code = ($this->dumpLoader ?? new MysqlDumpLoader())->dump(
                $database,
                $path,
                $threads,
                $output,
                $strategy["databaseInclude"] ?? [],
                $strategy["databaseExclude"] ?? [],
            );
        ...
        if ($code === Command::SUCCESS) {
            ...
            if (is_array($strategy)) {
                $metadata["databaseStrategy"] = $strategy["code"];
                $metadata["databaseStrategyTables"] = [
                    "include" => $strategy["databaseInclude"],
                    "exclude" => $strategy["databaseExclude"],
                ];
            }
            file_put_contents(
    ...
    private function resolveStrategy(InputInterface $input, OutputInterface $output): array|false|null {
        ...
        if (!is_string($code) || $code === "") {
            $this->writeMessage($output, "<error>Опция --strategy должна содержать код " . "стратегии БД.</error>");
            return false;
        }
