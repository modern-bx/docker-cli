...
{
    public function __construct(public string $name, public int $page, public int $pageSize, public string $backupName, public string $composition, public string $database, public string $location, public ?string $dateFrom, public ?string $dateTo, public string $sort, public string $direction)
    {
    }
    ...
    public static function fromRequest(RequestData $request): static
    {
        ...
        if ($page === false || !in_array($pageSize, [25, 50, 100], true) || !is_string($name) || mb_strlen($name) > 500 || !in_array($composition, ['all', 'database', 'files', 'database-files'], true) || !in_array($database, ['all', 'mysql', 'postgres'], true) || !is_string($location) || mb_strlen($location) > 100 || $dateFrom === false || $dateTo === false || ($dateFrom !== null && $dateTo !== null && $dateFrom > $dateTo) || !in_array($sort, ['name', 'date', 'composition', 'size', 'database', 'location'], true) || !in_array($direction, ['asc', 'desc'], true)) {
            throw new RequestValidationException('Некорректные параметры списка бэкапов.');
        }
        return new static(rawurldecode($request->route['name']), $page, $pageSize, trim($name), $composition, $database, trim($location), $dateFrom, $dateTo, $sort, $direction);
    }
