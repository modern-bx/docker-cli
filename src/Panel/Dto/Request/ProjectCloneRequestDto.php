...
final readonly class ProjectCloneRequestDto implements RequestDto
{
    ...
    public function __construct(public string $name, public ?string $to, public ?string $location, public bool $skipDb, public array $dbms)
    {
    }
    ...
    public static function fromRequest(RequestData $request): static
    {
        ...
        return new static($name, is_string($to) && trim($to) !== '' ? trim($to) : null, is_string($location) && trim($location) !== '' ? trim($location) : null, $skipDb, array_values(array_unique($dbms)));
    }
}
