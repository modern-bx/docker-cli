...
use DockerCli\Project\PostgresRecovery;
...
final class PostgresRecoverCommand extends AbstractCommand
{
    public function __construct(private readonly ?PostgresRecovery $recovery = null)
    {
        ...
        $this->addOption("from", null, InputOption::VALUE_REQUIRED, "Каталог data исходного PostgreSQL-инстанса.", ".");
        $this->addOption("to", null, InputOption::VALUE_REQUIRED, "Каталог для directory-дампов pg_dump.", ".");
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $databases =
            $databaseOption === null
                ? []
                : array_values(array_unique(array_filter(array_map("trim", explode(",", (string) $databaseOption)))));
        if ($databaseOption !== null && $databases === []) {
            $this->writeMessage(
                $output,
                "<error>Опция --database должна содержать хотя бы " . "одно имя базы.</error>",
            );
            return Command::INVALID;
        }
        try {
            ($this->recovery ?? new PostgresRecovery())->recover($from, $to, $databases, $output);
