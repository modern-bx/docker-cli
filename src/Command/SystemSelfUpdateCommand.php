...
final class SystemSelfUpdateCommand extends AbstractCommand {
    ...
    public function __construct(private readonly ?SystemCompose $compose = null) {
        ...
        $this->addOption(
            ...
            "Не собирать системные образы после " . "обновления конфигурации.",
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        if ($binary === "") {
            $this->writeMessage(
                $output,
                "<error>Самообновление доступно только при " . "запуске docker-cli из PHAR-архива.</error>",
            );
        ...
        try {
            ...
            if ($currentSize === $downloadedSize) {
                CommandContext::fromEnvironment($this, $output)->addMessage(
                    new Message(
                        "Обновление docker-cli не требуется: размер " . "PHAR-файла не изменился.",
                        notify: true,
                    ),
                );
            ...
            if ($status === Command::SUCCESS) {
                $status = $this->runCommand(
                    [$binary, "config:init", "--update", "--migrate", "--rebuild", "--force"],
                    $output,
                );
                if ($status !== Command::SUCCESS) {
                    $this->writeMessage(
                        $output,
                        "<comment>Обновление конфигурации завершилось " .
                            "с ошибкой; пробуем снова запустить " .
                            "систему.</comment>",
                    );
                    $this->runCommand(
                        $this->startCommand($binary, (bool) $input->getOption("no-rebuild-images")),
                        $output,
                    );
                } else {
                    $status = $this->runCommand(
            ...
            foreach ($runningUnits as $unit) {
                ...
                if ($status === Command::SUCCESS && $restartStatus !== Command::SUCCESS) {
    ...
    private function releaseUrl(SystemCompose $compose): string {
        ...
        return sprintf(
            "https://github.com/%s/%s/releases/download/%s-latest/%s.phar",
    ...
    private function validatePhar(string $file): void {
        try {
            ...
            throw new \RuntimeException(
                "Скачанный файл не является корректным PHAR-архивом: " . $exception->getMessage(),
                0,
                $exception,
            );
        }
    ...
    private function runningSystemdUnits(): array {
        [$status, $output] = $this->captureCommand([
            ...
            "--no-legend",
            "--plain",
            "docker-cli.panel.service",
            "docker-cli.queue.*.service",
        ]);
        if ($status !== Command::SUCCESS) {
            return [];
        }
        ...
        foreach (preg_split("/\R/", trim($output)) ?: [] as $line) {
            ...
            if (preg_match(self::SYSTEMD_UNIT_PATTERN, $unit) === 1) {
                $units[] = $unit;
            }
        }
    ...
    private function currentSystemdUnit(): ?string {
        foreach (@file("/proc/self/cgroup", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [] as $line) {
            if (
                preg_match(
                    '~(?:^|/)(docker-cli\.(?:panel|queue\.[a-zA-Z0-9_.@-]+)\.service)(?:/|$)~',
                    $line,
                    $matches,
                ) === 1
            ) {
