        ...
        foreach (glob(join_path($this->journalDirectory(), "*.jsonl")) ?: [] as $file) {
            ...
            while (($line = fgets($handle)) !== false) {
                try {
                    ...
                    if (
                        $command !== null &&
                        !str_contains(mb_strtolower((string) ($record["command"] ?? "")), mb_strtolower($command))
                    ) {
                        continue;
                    }
                    if (
                        $hook !== null &&
                        !str_contains(mb_strtolower((string) ($record["hook"] ?? "")), mb_strtolower($hook))
                    ) {
                        continue;
                    }
                    if ($timing !== null && ($record["timing"] ?? null) !== $timing) {
                        continue;
                    }
                    if ($hookLevel !== null && ($record["hookLevel"] ?? null) !== $hookLevel) {
                        continue;
                    }
                    $items[] = $record;
                } catch (\JsonException) {
