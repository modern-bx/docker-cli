...
final class SudoersManager
{
    ...
    public function setup(string $user, bool $update): string
    {
        ...
        if (file_exists($file) && !$update) {
            throw new \RuntimeException(sprintf('Конфигурация sudo для пользователя "%s" уже существует. Используйте --update для её обновления.', $user));
        }
        ...
        if ($temporary === false) {
            throw new \RuntimeException('Не удалось создать временный файл sudoers. Запустите команду через sudo.');
        }

        try {
            $content = sprintf("# Managed by docker-cli system:setup\n%s ALL=(root) NOPASSWD: %s\n", $user, implode(', ', self::COMMANDS));
    ...
    {
        $visudo = is_executable('/usr/sbin/visudo') ? '/usr/sbin/visudo' : (is_executable('/sbin/visudo') ? '/sbin/visudo' : null);
        if ($visudo === null) {
            throw new \RuntimeException('Команда visudo не найдена; конфигурация sudoers не была установлена.');
        }
        $process = proc_open([$visudo, '-c', '-f', $file], [
            ['file', '/dev/null', 'r'],
            ['file', '/dev/null', 'w'],
            STDERR,
        ], $pipes);
