...
final class TreeArchiveVolumes
{
    public static function parseSize(string $value): int
    {
        ...
        if ($bytes < 1 || $bytes > PHP_INT_MAX || floor($bytes) !== $bytes) {
            throw new \InvalidArgumentException(
                "Размер тома должен задавать целое " . "положительное количество байт.",
            );
        }
        return (int) $bytes;
    }
    ...
    public function split(string $directory, string $archiveName, ?int $chunkSize, ?int $chunkCount): array
    {
        ...
        if ($total === false) {
            throw new \RuntimeException("Не удалось определить размер файлового архива.");
        }
        if ($chunkCount !== null) {
            if ($chunkCount < 2) {
                throw new \InvalidArgumentException("Количество томов должно быть не меньше 2.");
            }
            if ($total < $chunkCount) {
                throw new \InvalidArgumentException("Количество томов не может превышать размер " . "архива в байтах.");
            }
            $chunkSize = (int) ceil($total / $chunkCount);
        }
        if ($chunkSize === null || $chunkSize < 1) {
            throw new \InvalidArgumentException("Размер тома должен быть положительным.");
        }

        $source = fopen($archive, "rb");
        if (!is_resource($source)) {
            throw new \RuntimeException("Не удалось открыть файловый архив для " . "разбиения на тома.");
        }
        $parts = [];
        try {
            for ($index = 1; !feof($source); $index++) {
                ...
                if (!is_resource($target)) {
                    throw new \RuntimeException(sprintf("Не удалось создать том «%s».", $name));
                }
                $written = stream_copy_to_stream($source, $target, $chunkSize);
                fclose($target);
                if ($written === false) {
                    throw new \RuntimeException(sprintf("Не удалось записать том «%s».", $name));
                }
                if ($written === 0) {
                    @unlink(join_path($directory, $name));
                    break;
                }
        ...
        if ($chunkCount !== null && count($parts) !== $chunkCount) {
            throw new \RuntimeException("Не удалось создать запрошенное количество томов.");
        }
        if (!unlink($archive)) {
            throw new \RuntimeException("Не удалось удалить исходный архив после " . "разбиения на тома.");
        }
    ...
    public function validate(string $directory, array $metadata): array
    {
        ...
        if ($volumes === null) {
            return is_file(join_path($directory, $archive)) ? [] : [sprintf("Отсутствует файл архива «%s».", $archive)];
        }
        if (
            !is_array($volumes) ||
            !is_int($volumes["chunkCount"] ?? null) ||
            !is_int($volumes["chunkSize"] ?? null) ||
            !is_array($volumes["parts"] ?? null) ||
            !array_is_list($volumes["parts"])
        ) {
            return ["Метаданные о томах повреждены или имеют " . "неизвестный формат."];
        }
        $errors = [];
        $parts = $volumes["parts"];
        if (count($parts) !== $volumes["chunkCount"]) {
            $errors[] = sprintf(
                "В метаданных перечислено %d частей, но " . "указано количество %d.",
                count($parts),
                $volumes["chunkCount"],
            );
        }
        foreach ($parts as $index => $part) {
            ...
            if (!is_file($path)) {
                $errors[] = sprintf("Не хватает части №%d «%s».", $index + 1, $part["name"]);
                continue;
            }
            $actual = filesize($path);
            if ($actual !== $part["size"]) {
                $errors[] = sprintf(
                    "Размер части №%d «%s» не совпадает: " . "ожидалось %d байт, найдено %d байт.",
                    $index + 1,
                    $part["name"],
                    $part["size"],
                    $actual === false ? 0 : $actual,
                );
            }
            if ($index < count($parts) - 1 && $part["size"] !== $volumes["chunkSize"]) {
                $errors[] = sprintf(
                    "Размер части №%d не равен указанному " . "размеру тома %d байт.",
                    $index + 1,
                    $volumes["chunkSize"],
                );
            }
        ...
        foreach (glob(join_path($directory, $archive . ".part*")) ?: [] as $path) {
            if (!in_array(basename($path), $expectedNames, true)) {
                $errors[] = sprintf("Найдена лишняя часть «%s», отсутствующая в метаданных.", basename($path));
            }
        }
    ...
    public function assemble(string $directory, array $metadata): string
    {
        ...
        if (!isset($metadata["volumes"])) {
            return join_path($directory, $metadata["archive"]);
        }
        $temporary = tempnam(sys_get_temp_dir(), "docker-cli-tree-volume-");
        if ($temporary === false) {
            throw new \RuntimeException("Не удалось создать временный файл для сборки томов.");
        }
        $target = fopen($temporary, "wb");
        try {
            if (!is_resource($target)) {
                throw new \RuntimeException("Не удалось открыть временный файл для сборки томов.");
            }
            foreach ($metadata["volumes"]["parts"] as $part) {
                $source = fopen(join_path($directory, $part["name"]), "rb");
                if (!is_resource($source) || stream_copy_to_stream($source, $target) === false) {
                    throw new \RuntimeException(sprintf("Не удалось прочитать том «%s».", $part["name"]));
                }
                fclose($source);
            }
            fclose($target);
