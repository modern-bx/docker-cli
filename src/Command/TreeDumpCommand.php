...
final class TreeDumpCommand extends AbstractCommand
{
    public function __construct(
        ...
        $this->addOption(
            ...
            "Архиватор: gzip, bzip2, xz, zstd, lz4 или zip.",
        );
        ...
        $this->addOption(
            ...
            null,
            InputOption::VALUE_REQUIRED,
            "Максимальный размер тома: например 10K, 1.5M или 2.25G.",
        );
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!is_string($name) || $name === "" || basename($name) !== $name || in_array($name, [".", ".."], true)) {
            $this->writeMessage(
                $output,
                "<error>Опция --name должна содержать корректное " . "короткое имя директории бэкапа.</error>",
            );
        ...
        if ($chunkSizeOption !== null && $chunkCountOption !== null) {
            $this->writeMessage(
                $output,
                "<error>Опции --chunk-size и --chunk-count нельзя " . "использовать одновременно.</error>",
            );
            return Command::INVALID;
        }
        try {
            ...
            $chunkCount =
                $chunkCountOption === null
                    ? null
                    : filter_var($chunkCountOption, FILTER_VALIDATE_INT, ["options" => ["min_range" => 2]]);
            if ($chunkCountOption !== null && $chunkCount === false) {
                throw new \InvalidArgumentException("Количество томов должно быть целым числом не меньше 2.");
            }
        ...
        if ($strategyCode !== null && (!is_string($strategyCode) || $strategyCode === "")) {
            $this->writeMessage(
                $output,
                "<error>Опция --strategy должна содержать код " . "файловой стратегии.</error>",
            );
            return Command::INVALID;
        }
        $strategy = null;
        if (is_string($strategyCode)) {
            ...
            if ($strategy === null) {
                $this->writeMessage(
                    $output,
                    sprintf("<error>Файловая стратегия с кодом «%s» не найдена.</error>", $strategyCode),
                );
                return Command::INVALID;
            }
        }
        ...
        try {
            ...
            if (is_string($input->getOption("comment")) && trim($input->getOption("comment")) !== "") {
                $metadata["comment"] = trim($input->getOption("comment"));
            }
            if ($chunkSize !== null || $chunkCount !== null) {
                $metadata["volumes"] = (new TreeArchiveVolumes())->split(
                    $backupDirectory,
                    $archive,
                    $chunkSize,
                    $chunkCount === false ? null : $chunkCount,
                );
            }
            if ($strategy !== null) {
            ...
            if (
                file_put_contents(
                    join_path($backupDirectory, "docker-cli.json"),
