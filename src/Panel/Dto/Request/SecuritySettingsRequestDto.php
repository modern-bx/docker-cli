    ...
    public static function fromRequest(RequestData $request): static
    {
        ...
        if (!is_int($hours) || $hours < 1 || $hours > SecuritySettingsRepository::MAX_SESSION_HOURS) {
            throw new RequestValidationException(
                sprintf(
                    "Длительность сессии должна быть от 1 до %d часов.",
                    SecuritySettingsRepository::MAX_SESSION_HOURS,
                ),
            );
        }
        ...
        if (str_contains($login, ":") || str_contains($login, "\n") || str_contains($login, "\r")) {
            throw new RequestValidationException(
                "Логин HTTP-авторизации не должен содержать " . "двоеточие или переносы строк.",
            );
        }
