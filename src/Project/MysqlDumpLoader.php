    ...
    public function dump(
        ...
        return $this->run(
            array_merge($compose->dockerComposeCommand("run"), [
                ...
                "mydumper",
                "-ec",
                'mydumper --host="$6" --user=root --password="${MYSQL_ROOT_PASSWORD:?}" ' .
                '--database="$1" --outputdir=/dump --threads="$2" ${5:+--regex="$5"} && chown -R ' .
                '"$3:$4" /dump',
                "sh",
                $database,
                (string) $threads,
    ...
    public function load(
        ...
        return $this->run(
            array_merge($compose->dockerComposeCommand("run"), [
                ...
                "mydumper",
                "-ec",
                'myloader --host="$4" --user=root --password="${MYSQL_ROOT_PASSWORD:?}" ' .
                '--database="$1" --directory=/dump --threads="$2" --drop-database ' .
                '--optimize-keys=AFTER_IMPORT_ALL_TABLES $3',
                "sh",
                $database,
                (string) $threads,
                $redoOption,
