...
final class DataDatabaseDeleteCommand extends AbstractCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($dbms === null || $items === []) { $output->writeln('<error>Укажите базы и поддерживаемую СУБД: mysql или postgres.</error>'); return Command::INVALID; }
        if (!$input->getOption('force') && !$this->getHelper('question')->ask($input, $output, new ConfirmationQuestion('Удалить указанные базы? [y/N] ', false))) { $output->writeln('<comment>Операция отменена.</comment>'); return Command::SUCCESS; }
