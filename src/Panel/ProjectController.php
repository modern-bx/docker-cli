...
final class ProjectController
{
    ...
    #[
        Route(
            "POST",
            "/api/projects/{name}/{action:" . ProjectActionEnum::ROUTE_PATTERN . "}",
            ProjectActionRequestDto::class,
        ...
        foreach ($this->projects->registeredProjectNames() as $name) {
            ...
            $projects[] = new ProjectDto(
                ...
                languageVersion: ($project["language"] ?? null) === "php"
                    ? (PhpLanguageVersion::isSupported($project["language_version"] ?? null)
                ...
                mysqlHost: is_string($databases["mysql"]["hostname"] ?? null) && $databases["mysql"]["hostname"] !== ""
                    ? $databases["mysql"]["hostname"]
                    : "docker-cli-mysql",
                postgresHost: is_string($databases["postgres"]["hostname"] ?? null) &&
                $databases["postgres"]["hostname"] !== ""
                    ? $databases["postgres"]["hostname"]
                    : "docker-cli-postgres",
    ...
    #[
        Route(
            "POST",
            "/api/projects/{name}/schedule/{index:\d+}",
            ProjectScheduleRequestDto::class,
    ...
    #[
        Route(
            "DELETE",
            "/api/projects/{name}/schedule/{index:\d+}",
            ProjectScheduleItemRequestDto::class,
            ScheduleListDto::class,
        ),
    ]
    ...
    private function writeSchedule(string $name, array $config, array $items): void
    {
        ...
        if (is_string($root) && $root !== "") {
            ...
            file_put_contents(
                ...
                Yaml::dump($repositoryConfig, 6, 2, Yaml::DUMP_MULTI_LINE_LITERAL_BLOCK),
            );
        }
    ...
    public function backups(ProjectBackupListRequestDto $request): ProjectBackupListDto
    {
        ...
        $grouped = [];
        $strategies = array_column(
            ($this->backupSettings ?? new BackupsSettingsRepository())->fileStrategies(),
            "name",
            "code",
        );
        ...
        foreach ($locations as $location) {
            foreach (["mysql" => "MySQL", "postgres" => "PostgreSQL"] as $databaseCode => $databaseName) {
                ...
                foreach (glob(join_path($directory, "*"), GLOB_ONLYDIR) ?: [] as $backup) {
                    ...
                    $metadata = is_file($metadataFile)
                        ? json_decode((string) file_get_contents($metadataFile), true)
                        : null;
                    if (
                        $location["code"] !== "" &&
                        (!is_array($metadata) || ($metadata["project"] ?? null) !== $request->name)
                    ) {
                        continue;
                    }
                    $createdAt =
                        is_array($metadata) && is_string($metadata["createdAt"] ?? null)
                            ? strtotime($metadata["createdAt"])
                            : false;
                    ...
                    if (!isset($grouped[$key])) {
                        $grouped[$key] = [
                            ...
                            "date" => gmdate(
                                DATE_ATOM,
                                $createdAt !== false ? $createdAt : ($timestamp === false ? 0 : $timestamp),
                            ),
                            "composition" => "БД",
                            "size" => 0,
                            "sizeParts" => [],
                            ...
                            "comment" =>
                                is_array($metadata) && is_string($metadata["comment"] ?? null)
                                    ? $metadata["comment"]
                                    : "",
                        ];
                    }
                    $databaseStrategyCode =
                        is_array($metadata) && is_string($metadata["databaseStrategy"] ?? null)
                            ? $metadata["databaseStrategy"]
                            : "";
                    $databaseStrategyNames = array_column(
                        ($this->backupSettings ?? new BackupsSettingsRepository())->fileStrategies(),
                        "name",
                        "code",
                    );
                    $grouped[$key]["databaseStrategyCode"] = $databaseStrategyCode;
                    $grouped[$key]["databaseStrategy"] =
                        $databaseStrategyCode !== "" ? $databaseStrategyNames[$databaseStrategyCode] ?? null : null;
                    $grouped[$key]["databaseStrategyTables"] = is_array($metadata["databaseStrategyTables"] ?? null)
                        ? $metadata["databaseStrategyTables"]
                        : ["include" => [], "exclude" => []];
                    ...
                    if (
                        $grouped[$key]["comment"] === "" &&
                        is_array($metadata) &&
                        is_string($metadata["comment"] ?? null)
                    ) {
                        $grouped[$key]["comment"] = $metadata["comment"];
                    }
                }
            }
            ...
            foreach (glob(join_path($directory, "*"), GLOB_ONLYDIR) ?: [] as $backup) {
                ...
                $strategyPaths = is_array($metadata["strategyPaths"] ?? null)
                    ? $metadata["strategyPaths"]
                    : ["include" => [], "exclude" => []];
                $fileSize = $this->directorySize($backup);
                $volumeCount =
                    is_array($metadata["volumes"] ?? null) && is_int($metadata["volumes"]["chunkCount"] ?? null)
                        ? $metadata["volumes"]["chunkCount"]
                        : 1;
                $fileData = [
                    "name" => basename($backup),
                    "date" => gmdate(
                    ...
                    "size" => $fileSize,
                    "sizeParts" => [
                        ["type" => "files", "name" => "Файлы", "size" => $fileSize, "volumeCount" => $volumeCount],
                    ],
                    "database" => null,
                    "databaseCode" => "",
                ...
                if (isset($grouped[$key])) {
                    ...
                    if ($fileData["comment"] !== "") {
                        $grouped[$key]["comment"] = $fileData["comment"];
                    }
                } else {
        ...
        $items = array_values(
            array_filter($items, static function (array $item) use ($request): bool {
                $date = substr($item["date"], 0, 10);
                return ($request->backupName === "" ||
                    str_contains(mb_strtolower($item["name"]), mb_strtolower($request->backupName))) &&
                    ($request->composition === "all" ||
                        ($request->composition === "database" && $item["composition"] === "БД") ||
                        ($request->composition === "files" && $item["composition"] === "Файлы") ||
                        ($request->composition === "database-files" && $item["composition"] === "БД и файлы")) &&
                    ($request->database === "all" || in_array($request->database, $item["databaseCodes"], true)) &&
                    ($request->strategy === "all" ||
                        ($request->strategy === "none"
                            ? $item["strategyCode"] === ""
                            : $request->strategy === $item["strategyCode"])) &&
                    ($request->location === "all" ||
                        ($request->location === "project"
                            ? $item["location"] === ""
    ...
    public function createBackup(ProjectBackupCreateRequestDto $request): QueuedOperationDto
    {
        ...
        if (!$request->database && !$request->files) {
            throw new ProjectActionException("Выберите данные для создания бэкапа.", 422);
        }
        if ($request->database && !$request->mysql && !$request->postgres) {
            throw new ProjectActionException("Выберите хотя бы одну базу данных.", 422);
        }
        if (
            $request->location !== "" &&
            !in_array(
                $request->location,
                array_column(($this->backupSettings ?? new BackupsSettingsRepository())->locations(), "code"),
                true,
            )
        ) {
        ...
        if (
            ($request->files || $request->database) &&
            $request->strategy !== "" &&
            !in_array(
                $request->strategy,
                array_column(($this->backupSettings ?? new BackupsSettingsRepository())->fileStrategies(), "code"),
                true,
            )
        ) {
        ...
        $operationCode = $request->database
            ? ($request->mysql
                ? "core.mysql.dump"
                : "core.postgres.dump")
            : "core.tree.dump";
    ...
    #[
        Route(
            "POST",
            "/api/projects/{name}/backups/{backup}/comment",
            ProjectBackupCommentRequestDto::class,
    ...
    public function updateBackupComment(ProjectBackupCommentRequestDto $request): BackupCommentDto
    {
        ...
        if ($request->location !== "") {
            ...
            if (!is_array($location)) {
                throw new ProjectActionException("Хранилище бэкапов не найдено.", 404);
            }
    ...
    #[
        Route(
            "POST",
            "/api/projects/{name}/backups/{backup}/restore",
            ProjectBackupRestoreRequestDto::class,
    ...
    public function restoreBackup(ProjectBackupRestoreRequestDto $request): QueuedOperationDto
    {
        ...
        if ($this->projects->isProjectProtected($request->name)) {
            throw new ProjectActionException("Проект защищен.", 409);
        }
        $config = $this->projects->readProjectConfig($request->name);
        $root = $config["data"]["project"]["root"] ?? null;
        ...
        $tasks = [];
        $databases =
            $request->databases !== [] ? $request->databases : ($request->database !== "" ? [$request->database] : []);
        ...
        if ($request->files) {
            ...
            $volumeErrors = is_array($metadata)
                ? (new TreeArchiveVolumes())->validate($backup, $metadata)
                : ["Метаданные файлового бэкапа повреждены."];
            if ($volumeErrors !== []) {
                throw new ProjectActionException("Файловый бэкап повреждён: " . implode(" ", $volumeErrors), 422);
            }
            $tasks[] = [
                "code" => "core.tree.load",
                "arguments" => [
    ...
    private function backupPath(
        ...
        if (
            $backup === false ||
            !is_dir($backup) ||
            !str_starts_with($backup . DIRECTORY_SEPARATOR, $root . DIRECTORY_SEPARATOR)
        ) {
            throw new ProjectActionException("Бэкап не найден.", 404);
        }
        if ($checkMetadata) {
    ...
    #[
        Route(
            "POST",
            "/api/projects/{name}/backups/{backup}/delete",
            ProjectBackupRestoreRequestDto::class,
            QueuedOperationDto::class,
        ),
    ]
    ...
    public function options(EmptyRequestDto $request): ProjectOptionsDto
    {
        ...
        foreach (($this->tasks ?? new TaskRepository())->all() as $definition) {
            ...
            if (($task["context"] ?? null) !== "project" || !in_array("project:init", $task["tags"] ?? [], true)) {
                continue;
            }
    ...
    public function create(ProjectCreateRequestDto $request): ProjectListDto
    {
        ...
        if (preg_match('/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/', $name) !== 1) {
            throw new ProjectActionException(
                "Код проекта должен содержать строчные " . "латинские буквы, цифры и дефисы.",
                422,
            );
        }
        if ($this->projects->hasProject($name) || file_exists(join_path($location["path"], $name))) {
            throw new ProjectActionException("Проект или директория с таким кодом уже существует.", 409);
        }
        $arguments = [
        ...
        if ($request->dedicatedDatabases !== []) {
            ...
            foreach ($request->dedicatedDatabases as $driver) {
                ...
                if ($databaseLocation !== null) {
                    $arguments["location-" . $driver] = [
                        "value" => join_path($databaseLocation, $driver . "-" . $name),
                    ];
                }
            }
        }
        ...
        if ($request->deploymentScript !== null) {
            ...
            if (!($script instanceof DeploymentScriptDto)) {
                throw new ProjectActionException("Скрипт развертки не найден.", 422);
            }
            $scriptArguments = [];
            foreach ($request->deploymentArguments as $argumentName => $value) {
                if (!is_string($argumentName)) {
                    throw new ProjectActionException("Некорректные аргументы скрипта развертки.", 422);
                }
                if (
                    $value === "" &&
                    isset($script->parameters[$argumentName]) &&
                    ($script->parameters[$argumentName]["required"] ?? false) !== true
                ) {
                    continue;
                }
        ...
        if ($validationErrors !== []) {
            throw new ProjectActionException(implode("\n", $validationErrors), 422);
        }
        try {
            ($this->queues ?? new QueueRepository())->create("default", "core.project.up", $item);
        } catch (\InvalidArgumentException | \RuntimeException $exception) {
            throw new ProjectActionException($exception->getMessage(), 500);
        }
        return $this->projects(new EmptyRequestDto());
    ...
    public function clone(ProjectCloneRequestDto $request): QueuedOperationDto
    {
        ...
        if ($request->location === null) {
            $arguments["here"] = ["value" => true];
        } else {
            $arguments["location"] = ["value" => $request->location];
        }
        if ($request->skipDb || $request->dbms === []) {
            $arguments["skip-db"] = ["value" => true];
        } else {
            $arguments["dbms"] = ["value" => implode(",", $request->dbms)];
        }
        if ($request->mirror !== null) {
            $arguments["mirror"] = ["value" => implode(",", $request->mirror)];
        }
        if ($request->dedicatedDatabases !== null) {
            $arguments["dedicated-db"] = [
                "value" => $request->dedicatedDatabases === [] ? "false" : implode(",", $request->dedicatedDatabases),
            ];
            $options = $this->options(new EmptyRequestDto());
            foreach ($request->dedicatedDatabases as $driver) {
                $selection = $driver === "mysql" ? $request->locationMysql : $request->locationPostgres;
                ...
                $arguments["location-" . $driver] = [
                    "value" =>
                        $databaseLocation === null
                            ? "system"
                            : join_path($databaseLocation, $driver . "-" . $targetName),
                ];
            }
        }
        $item = [
        ...
        try {
            $file = ($this->queues ?? new QueueRepository())->create("default", "core.project.clone", $item);
        } catch (\InvalidArgumentException | \RuntimeException $exception) {
            throw new ProjectActionException($exception->getMessage(), 500);
        }
        return new QueuedOperationDto($file);
    ...
    public function update(ProjectUpdateRequestDto $request): ProjectListDto
    {
        ...
        if ($request->name !== null && preg_match('/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/', $request->name) !== 1) {
            throw new ProjectActionException(
                "Имя проекта должно содержать строчные " . "латинские буквы, цифры и дефисы.",
                422,
            );
        }
        if (
            ...
            $this->projects->hasProject($request->name)
        ) {
            throw new ProjectActionException("Проект с таким именем уже существует.", 409);
        }
        $options = $this->options(new EmptyRequestDto());
        if ($request->language !== null && !isset($options->frameworks[$request->language])) {
            throw new ProjectActionException("Язык не поддерживается.", 422);
        }
        $language = $request->language ?? "php";
        if (
            $request->languageVersion !== null &&
            ($language !== "php" || !PhpLanguageVersion::isSupported($request->languageVersion))
        ) {
            throw new ProjectActionException("Версия языка не поддерживается.", 422);
        }
        if (
        ...
        if ($request->dedicatedDatabases !== null) {
            ...
            foreach ($request->dedicatedDatabases as $driver) {
                ...
                $arguments["location_" . $driver] = [
                    "value" => $location === null ? "system" : join_path($location, $driver . "-" . $targetName),
                ];
            }
        }
        $item = [
        ...
        try {
            ($this->queues ?? new QueueRepository())->create("default", "core.project.update", $item);
        } catch (\InvalidArgumentException | \RuntimeException $exception) {
            throw new ProjectActionException($exception->getMessage(), 500);
        }
        return $this->projects(new EmptyRequestDto());
    ...
    private function reloadOpenResty(): void
    {
        ...
        if (!is_resource($process)) {
            throw new ProjectActionException("Не удалось перезагрузить конфигурацию OpenResty.");
        }
