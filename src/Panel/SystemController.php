...
final class SystemController {
    ...
    public function selfUpdate(EmptyRequestDto $request): QueuedOperationDto {
        ...
        if ($validationErrors !== []) {
            throw new SystemActionException(implode("\n", $validationErrors), 422);
        }
        try {
    ...
    #[
        Route(
            "POST",
            "/api/system/{action:" . SystemActionEnum::ROUTE_PATTERN . "}",
            SystemActionRequestDto::class,
    ...
    #[
        Route(
            "POST",
            "/api/system/services/{service}/{action:" . SystemActionEnum::ROUTE_PATTERN . "}",
            SystemActionRequestDto::class,
            SystemStatusDto::class,
        ),
    ]
    ...
    private function configuredServices(): array {
        ...
        if ($code !== 0 || !is_array($config)) {
            throw new SystemActionException(
                $output !== "" ? $output : "Не удалось прочитать конфигурацию Docker Compose.",
            );
        }
    ...
    private function runningServices(): array {
        ...
        foreach ($rows as $row) {
            if (
                ...
                strtolower((string) ($row["State"] ?? "")) === "running"
            ) {
    ...
    private function run(array $arguments, bool $fail = true): array {
        ...
        $process = proc_open(
            ...
            $pipes,
            $this->compose->directory(),
            $this->compose->dockerProcessEnvironment(),
        );
        if (!is_resource($process)) {
            throw new SystemActionException("Не удалось запустить Docker Compose.");
        }
        ...
        if ($fail && $code !== 0) {
            throw new SystemActionException($output !== "" ? $output : "Docker Compose завершился с ошибкой.");
        }
