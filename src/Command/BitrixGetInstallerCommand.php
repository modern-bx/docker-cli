...
final class BitrixGetInstallerCommand extends AbstractCommand {
    ...
    public function __construct() {
        ...
        $this->addOption(
            ...
            "Редакция продукта (по умолчанию: start для " . "bitrix, business для bitrix24).",
        );
    ...
    private function download(string $url, string $target): void {
        ...
        $read = @fopen(
            ...
            "rb",
            false,
            stream_context_create(["http" => ["user_agent" => "BitrixSiteLoader", "ignore_errors" => true]]),
        );
        if (!is_resource($read)) {
            @unlink($temporary);
    ...
    private function runProcess(array $command): void {
        ...
        if ($exitCode !== 0) {
            throw new \RuntimeException(
                sprintf("Команда завершилась с кодом %d: %s", $exitCode, implode(" ", $command)),
            );
        }
    }
