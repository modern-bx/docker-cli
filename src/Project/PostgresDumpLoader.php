...
final class PostgresDumpLoader
{
    ...
    public function dump(
        ...
        if (file_exists($directory) && !is_dir($directory)) {
            $output->writeln(
                sprintf('<error>Путь "%s" уже существует и не является ' . "директорией.</error>", $directory),
            );
        ...
        foreach ($include as $pattern) {
            $filters[] = "--table=" . $pattern;
        }
        foreach ($exclude as $pattern) {
            $filters[] = "--exclude-table=" . $pattern;
        }
        return $this->run(
            array_merge($compose->dockerComposeCommand("run"), [
                "--rm",
                "-T",
                "--no-deps",
                "--user",
                ...
                "postgres",
                "-ec",
                'export PGPASSWORD="${POSTGRES_PASSWORD:?}"; database="$1"; jobs="$2"; uid="$3"; ' .
                'gid="$4"; hostname="$5"; shift 5; pg_dump --host="$hostname" ' .
                '--username="${POSTGRES_USER:-system}" --format=directory --jobs="$jobs" ' .
                '--file=/dump "$database" "$@"; chown -R "$uid:$gid" /dump',
                "sh",
                $database,
                (string) $jobs,
                (string) $this->uid(),
    ...
    public function load(
        ...
        return $this->run(
            array_merge($compose->dockerComposeCommand("run"), [
                ...
                pg_restore --host="$4" --username="$root_user" --dbname="$1" --jobs="$3" \
                    --no-owner --no-acl --role="$2" /dump
                SH
                ,
                "sh",
                $database,
