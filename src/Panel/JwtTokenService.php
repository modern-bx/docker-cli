...
final class JwtTokenService {
    ...
    public function login(string $token, ?int $now = null): ?string {
        ...
        try {
            if (UserRepository::normalizeLogin($claims["sub"]) !== $claims["sub"]) {
                return null;
            }
        ...
        return $this->repository->contains($token, $claims["jti"], $claims["sub"], $claims["exp"], $now)
            ? $claims["sub"]
            : null;
    }
