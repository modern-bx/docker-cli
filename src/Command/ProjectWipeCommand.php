...
final class ProjectWipeCommand extends AbstractCommand {
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        if ($projectName === null) {
            $this->writeMessage(
                $output,
                "<error>Укажите код проекта через --project или " .
        ...
        if (!is_dir($projectRoot . DIRECTORY_SEPARATOR . ".docker-cli")) {
            $this->writeMessage(
                $output,
                sprintf(
                    '<error>В директории проекта "%s" не найдена ' . "директория .docker-cli.</error>",
                    $projectName,
                ),
            );
            return Command::FAILURE;
        ...
        try {
            ...
            $this->writeMessage(
                $output,
                sprintf('<error>Не удалось очистить проект "%s": %s</error>', $projectName, $exception->getMessage()),
            );
            return Command::FAILURE;
        }
        ...
        ($this->context ?? CommandContext::fromEnvironment($this, $output))->addMessage(
            new Message(
                sprintf(
                    "Файлы проекта **%s** успешно удалены. " . "Служебная директория `.docker-cli` сохранена.",
                    $projectName,
                ),
                notify: true,
            ),
