...
final class PanelUpCommand extends AbstractCommand
{
    public function __construct(private readonly SystemdService $systemdService = new SystemdService())
    {
        ...
        $this->addOption(
            ...
            "Порт HTTP-сервера (переопределяет PANEL_PORT из .env).",
        );
        ...
        $this->addOption(
            ...
            null,
            InputOption::VALUE_REQUIRED,
            "Пользователь, от имени которого будет " . "работать systemd-сервис.",
        );
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($input->getOption("user") !== null || $input->getOption("path") !== null) {
            $this->writeMessage(
                $output,
                "<error>Опции --user и --path можно использовать " . "только вместе с -d.</error>",
            );
            return Command::INVALID;
        }
        ...
        if ($salt === "" || $jwtSecret === "") {
            $this->writeMessage(
                $output,
                "<error>Секреты панели не настроены. Выполните " . "`docker-cli config:init`.</error>",
            );
            return Command::FAILURE;
        }
        ...
        try {
            ...
            $this->writeMessage(
                $output,
                "<error>Не удалось запустить HTTP-сервер: " . $exception->getMessage() . "</error>",
            );
            return Command::FAILURE;
        }
        ...
        $projects = new ProjectController(
            ...
            $compose,
            $queues,
            new ProjectsSettingsRepository(),
            new TaskRepository(),
            $backupsSettings,
        );
        ...
        $router = new Router(
            [
                new AuthController($users, $tokens, $tokenRepository),
                new \DockerCli\Panel\SecuritySettingsController($securitySettings),
                new \DockerCli\Panel\ProjectsSettingsController($projectsSettings),
                new BackupsSettingsController($backupsSettings),
                new \DockerCli\Panel\HooksSettingsController(new \DockerCli\Panel\HookRepository(), $projectsSettings),
                new \DockerCli\Panel\UsersSettingsController(
                    $users,
                    $tokenRepository,
                    new \DockerCli\Panel\PanelPasswordGenerator(),
                ),
                $state,
                $projects,
                $system,
        ...
        $this->writeMessage(
            $output,
            sprintf("<info>Панель запущена на https://panel.%s</info>", $compose->envValue("BASE_HOST", "")),
        );
        Loop::run();
    ...
    private function installSystemdService(InputInterface $input, OutputInterface $output, ?int $port): int
    {
        ...
        if (
            $rawPath !== null &&
            (!str_starts_with($binary, DIRECTORY_SEPARATOR) || !is_file($binary) || !is_executable($binary))
        ) {
            $this->writeMessage(
                $output,
                "<error>Опция --path должна указывать на " . "существующий исполняемый файл.</error>",
            );
            return Command::INVALID;
        }
        try {
        ...
        if (is_string($rawUser)) {
            ...
            $this->writeMessage(
                $output,
                sprintf(
                    "<info>Пользователю разрешено запускать, " .
                        "останавливать и перезапускать сервис %s без " .
                        "sudo.</info>",
                    $rawUser,
                ),
            );
        ...
        $this->writeMessage(
            $output,
            sprintf(
                "<info>Сервис включён и запущен. Управление: " . "systemctl {status|start|restart|stop} %s</info>",
                SystemdService::NAME,
            ),
        );
    ...
    private function configureGateway(SystemCompose $compose, int $port): void
    {
        ...
        $contents = sprintf("upstream panel_backend {\n    server host.docker.internal:%d;\n}\n", $port);
        ...
        $process = proc_open(
            ["docker", "exec", "docker-cli-panel-gateway", "nginx", "-s", "reload"],
