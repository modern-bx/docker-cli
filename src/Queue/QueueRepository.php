...
final class QueueRepository
{
    ...
    public function create(string $queue, string $code, array $item): string
    {
        ...
        for ($counter = 0; $counter <= 999; ++$counter) {
            $file = join_path(
                $this->queueDirectory($queue),
                "10-pending",
                sprintf("%d.%03d.%s.yaml", $timestamp, $counter, $safeCode),
            );
            $handle = @fopen($file, "x");
            if ($handle === false) {
                continue;
        ...
        foreach (["10-pending", "20-active", "40-failure", "50-error"] as $status) {
            ...
            foreach (array_filter($files, "is_file") as $file) {
                ...
                $items[] = [
                    ...
                    "queuedAt" => sprintf(
                        "%s.%06dZ",
                        gmdate("Y-m-d\TH:i:s", intdiv($microseconds, 1_000_000)),
    ...
    private function sortTimestampedFiles(array &$files): void
    {
        usort($files, function (string $left, string $right): int {
            ...
            return [$this->timestampMicroseconds($leftTimestamp, $left), basename($left)] <=> [
                $this->timestampMicroseconds($rightTimestamp, $right),
                basename($right),
            ];
        });
    }
    ...
    public function delete(string $queue, string $file): void
    {
        ...
        foreach (array_diff(self::STATUSES, ["20-active"]) as $status) {
            ...
            if (is_file($path)) {
                if (!unlink($path)) {
                    throw new \RuntimeException("Не удалось удалить элемент очереди.");
                }
                return;
            }
        }
    ...
    public function archive(string $queue, string $file): void
    {
        ...
        foreach (array_diff(self::STATUSES, ["20-active", "90-archive"]) as $status) {
            ...
            if (!is_array($item)) {
                throw new \RuntimeException("Некорректный элемент очереди.");
            }
    ...
    public function trace(
        ...
        $record = [
            ...
            "timestamp" => sprintf(
                "%s.%sZ",
                gmdate("Y-m-d\TH:i:s", (int) $seconds),
    ...
    private function appendLog(string $queue, array $record): void
    {
        ...
        if (
            file_put_contents(
                join_path($this->configDirectory(), "journal", "queue", $queue . ".jsonl"),
                $line,
                FILE_APPEND | LOCK_EX,
            ) === false
        ) {
        ...
        foreach (glob(join_path($this->configDirectory(), "journal", "queue", "*.jsonl")) ?: [] as $file) {
            ...
            while (($line = fgets($handle)) !== false) {
                try {
                    ...
                    if ($projectsFilter !== [] && array_intersect($projectsFilter, $recordProjects) === []) {
                        continue;
                    }
                    if ($statuses !== [] && !in_array($record["status"] ?? null, $statuses, true)) {
                        continue;
                    }
                    if (
                        $queueItem !== null &&
                        !str_contains(mb_strtolower((string) ($record["queueItem"] ?? "")), mb_strtolower($queueItem))
                    ) {
                        continue;
                    }
                    if (
                        $itemCode !== null &&
                        !str_contains(mb_strtolower((string) ($record["itemCode"] ?? "")), mb_strtolower($itemCode))
                    ) {
                        continue;
                    }
                    if (
                        $taskCode !== null &&
                        !str_contains(mb_strtolower((string) ($record["taskCode"] ?? "")), mb_strtolower($taskCode))
                    ) {
                        continue;
                    }
                    if ($levels !== [] && !in_array($record["level"] ?? null, $levels, true)) {
                        continue;
                    }
                    if ($contexts !== [] && !in_array($record["context"] ?? null, $contexts, true)) {
                        continue;
                    }
                    $items[] = $record;
                } catch (\JsonException) {
