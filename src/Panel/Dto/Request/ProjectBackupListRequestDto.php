...
final readonly class ProjectBackupListRequestDto implements RequestDto {
    ...
    public static function fromRequest(RequestData $request): static {
        ...
        if (
            $page === false ||
            !in_array($pageSize, [25, 50, 100], true) ||
            !is_string($name) ||
            mb_strlen($name) > 500 ||
            !in_array($composition, ["all", "database", "files", "database-files"], true) ||
            !in_array($database, ["all", "mysql", "postgres"], true) ||
            !is_string($strategy) ||
            mb_strlen($strategy) > 100 ||
            !is_string($location) ||
            mb_strlen($location) > 100 ||
            $dateFrom === false ||
            $dateTo === false ||
            ($dateFrom !== null && $dateTo !== null && $dateFrom > $dateTo) ||
            !in_array($sort, ["name", "date", "composition", "size", "database", "strategy", "location"], true) ||
