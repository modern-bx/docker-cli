...
final class PlayRunCommand extends AbstractCommand
{
    public function __construct()
    {
        ...
        $this->addArgument(
            ...
            "Путь к js-сценарию относительно " .
                "~/.config/docker-cli/actions/playwright/scripts; расширение .js " .
                "можно опустить.",
        );
        $this->addOption(
            "show",
            ...
            "Показывать управляемый браузер в окне " . "через локальный noVNC viewer.",
        );
        $this->addOption(
            "browser",
            null,
            InputOption::VALUE_REQUIRED,
            "Браузер для выполнения сценария: chromium, firefox или webkit.",
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($projectName === null) {
            $this->writeMessage(
                $output,
                "<error>Команда play:run выполняется только из " .
        ...
        if ($script === null) {
            $this->writeMessage(
                $output,
                "<error>Некорректное имя сценария: используйте " .
                    "относительный путь внутри playwright/scripts.</error>",
            );
        ...
        if (!is_file($scriptPath)) {
            $this->writeMessage(
                $output,
                sprintf(
                    '<error>Сценарий "%s" не найден в "%s".</error>',
                    $script,
                    $compose->playwrightScriptsDirectory(),
                ),
            );
        ...
        if ($browser !== null && !in_array($browser, ["chromium", "firefox", "webkit"], true)) {
            $this->writeMessage(
                $output,
                "<error>Некорректный браузер. Допустимые " . "значения: chromium, firefox, webkit.</error>",
            );
        ...
        try {
            ...
            $this->writeMessage(
                $output,
                sprintf("<error>Не удалось подготовить данные Playwright: %s</error>", $exception->getMessage()),
            );
        ...
        $command = array_merge($compose->dockerComposeCommand("run"), $runArguments, [
            ...
            "sh",
            "-lc",
            "mkdir -p /docker-cli/playwright/runtime && cd /docker-cli/playwright/runtime && " .
            "if [ ! -d node_modules/playwright ]; then npm --silent --no-update-notifier " .
            '--no-fund install playwright@${PLAYWRIGHT_VERSION:-1.61.0}; fi && ' .
            'NODE_PATH=/docker-cli/playwright/runtime/node_modules node "$@"',
     ...
     */
    private function writeContextFile(
        string $directory,
        string $projectRoot,
        array $projectConfig,
        ...
        foreach ($files as $file) {
            ...
            if (preg_match('/^[A-Za-z_$][A-Za-z0-9_$]*$/', $name) !== 1) {
                throw new \RuntimeException(
                    sprintf('Имя файла "%s" не является допустимым ' . "JavaScript-идентификатором.", basename($file)),
                );
            }
    ...
    private function openViewerWhenReady(string $url, int $port): void
    {
        ...
        $script = sprintf(
            'command -v %1$s >/dev/null 2>&1 || exit 0; while ! (echo ' .
                '>/dev/tcp/127.0.0.1/%2$d) >/dev/null 2>&1; do sleep 1; done; %1$s %3$s ' .
                ">/dev/null 2>&1",
