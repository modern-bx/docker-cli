...
Команда определяет фреймворк, создает запись `~/.config/docker-cli/projects/my-project/project.yaml` с именем проекта, кодовым именем фреймворка и корнем проекта, а также записывает метаданные проекта в `.docker-cli.yaml` в корне найденного проекта. Если проект с таким именем уже зарегистрирован или фреймворк не определен, команда завершается ошибкой.
...
Команда читает имя проекта из `.docker-cli.yaml`, удаляет соответствующую директорию в `~/.config/docker-cli/projects`, а затем удаляет `.docker-cli.yaml`. Если фреймворк не определен или файл метаданных отсутствует, команда завершается ошибкой.
