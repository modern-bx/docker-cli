...
Команды создают уведомления только при запуске внутри задачи (`task:run`), независимо от того, была задача запущена напрямую или обработчиком очереди. Прямой запуск команды, например `docker-cli project:wipe`, уведомление не создаёт.
