...
GlitchTip доступен по адресу `https://glitchtip.<ваш-домен>`. Он использует отдельную базу `glitchtip` в общем
системном PostgreSQL и собственный контейнер Valkey. Данные Valkey и загруженные файлы сохраняются в отдельных
именованных Docker volumes, а база GlitchTip — вместе с остальными данными системного PostgreSQL.

Команда `config:seed` генерирует секрет приложения и пароль администратора. Логин и пароль
