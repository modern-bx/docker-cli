...
Обязательные поля задачи: `name`, `code`, `type` и `action`. Сейчас единственный поддерживаемый тип задачи — `shell`. Поля `description`, `context` и `tags` опциональны. Теги задачи и параметров — списки символьных кодов на латинице; допускаются также цифры, `.`, `_` и `-` после первой буквы.

Параметры поддерживают типы `string`, `integer` и `list`. Для `integer` можно задать `min` и `max`, для `list` — варианты `items` с полями `name` и `value`. Поля параметра `name`, `description`, `required` и `tags` опциональны.
