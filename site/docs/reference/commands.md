...
По умолчанию клон наследует настройки выделенных БД исходного проекта. Для каждой
настроенной таким образом СУБД создаётся отдельный инстанс целевого проекта; если
выделенные БД у источника не настроены, новые инстансы не создаются. Набор можно
переопределить через `--dedicated-db=mysql,postgres`, а каталоги данных — через
`--location-mysql` и `--location-postgres` (значение `system` оставляет данные в
системном каталоге docker-cli). Значение `--dedicated-db=false`
принудительно размещает все БД клона в системных инстансах.
...
Выводит содержимое `project.yaml` текущего проекта без преобразований. Проект
определяется из его корня или любой поддиректории независимо от глубины. Кодовое имя
проекта можно указать явно, чтобы вызвать команду из любой другой директории.
...
Для задач с `context: project` проект определяется по текущей директории; `--project` позволяет
указать его явно. `--no-delete` оставляет скомпилированный файл после выполнения и печатает его путь.
