...
final readonly class StateController {
    public function __construct(
        private ProjectController $projects,
    ...
    public function state(EmptyRequestDto $request): PanelStateDto {
        return new PanelStateDto(
            $this->projects->projects($request)->projects,
            $this->system->status($request),
            $this->queue->state($request),
            $this->notifications->current($request),
        );
    }
}
