...
abstract class SystemUserCommand extends AbstractCommand
{
    ...
    private function invokingUser(): string
    {
        ...
        if (is_string($sudoUser) && $sudoUser !== '' && $sudoUser !== 'root') return $sudoUser;
        if (function_exists('posix_geteuid') && function_exists('posix_getpwuid')) {
            $account = posix_getpwuid(posix_geteuid());
            if (is_array($account) && is_string($account['name'] ?? null)) return $account['name'];
        }
        $user = trim((string) shell_exec('id -un 2>/dev/null'));
        if ($user === '') throw new \RuntimeException('Не удалось определить текущего пользователя. Укажите --user.');
