...
Системные интерфейсы ProxySQL и ProxyWeb доступны по адресам `https://proxysql.<домен>` и
`https://proxyweb.<домен>`. Команда `config:seed` создаёт отдельные учётные данные в системном `.env` для
администрирования ProxySQL, его Web UI и входа в ProxyWeb. ProxyWeb уже содержит подключение к системному ProxySQL, где при первом
запуске зарегистрированы системные серверы MySQL и PostgreSQL.
При добавлении, клонировании, изменении или удалении проекта docker-cli автоматически синхронизирует с ProxySQL
системные и выделенные серверы, пользователей и правила маршрутизации для каждой пары логин/база данных.
...
контейнеров, включая отдельный маршрут API для входа и создания учётной записи, а также разрешённый внешний
host DBTrail. PostgreSQL к DBTrail не подключается.
Таблицы MySQL без первичного ключа или не на InnoDB автоматически исключаются из наблюдения: DBTrail продолжает
работать с остальными таблицами проекта без изменения схемы, но история исключённых таблиц недоступна. Для больших
инсталляций Битрикс контейнер запускается с увеличенным лимитом аргументов, необходимым для полного списка таблиц.
