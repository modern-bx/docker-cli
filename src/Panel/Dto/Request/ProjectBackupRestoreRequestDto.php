...
final readonly class ProjectBackupRestoreRequestDto implements RequestDto {
    ...
    public static function fromRequest(RequestData $request): static {
        ...
        if (
            !in_array($database, ["", "mysql", "postgres"], true) ||
            !is_string($location) ||
            !is_bool($files) ||
            !is_bool($force) ||
            !is_bool($wipe) ||
            !is_array($databases) ||
            !array_is_list($databases) ||
            array_diff($databases, ["mysql", "postgres"]) !== [] ||
            count(array_unique($databases)) !== count($databases) ||
            ($databases === [] && !$files) ||
        ...
        return new static(
            ...
            rawurldecode($request->route["backup"]),
            $database,
            $location,
            $files,
            $force,
