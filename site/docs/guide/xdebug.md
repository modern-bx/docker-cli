...
Все PHP-FPM-образы устанавливают расширение Xdebug и включают только режим пошаговой отладки. Профайлинг,
трассировка, coverage и остальные режимы не включаются. Сборкой расширения управляет общий флаг
`PHP_ENABLE_XDEBUG` в системном `.env`; значение по умолчанию — `1`, а после изменения требуется пересборка образов.
