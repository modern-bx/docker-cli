...
final class BackupsSettingsRepository {
     ...
     * @return array{
     *     locations: list<array{path: string, code: string, default: bool}>,
    ...
    public function save(array $locations, array $fileStrategies): array {
        ...
        foreach ($locations as $index => &$location) {
            ...
            if (isset($used[$location["code"]])) {
                throw new \InvalidArgumentException("Коды расположений должны быть уникальными.");
            }
        ...
        foreach ($locations as $location) {
            ...
            if (!is_dir($path)) {
                throw new \InvalidArgumentException(
                    sprintf("Путь «%s» не существует или не является каталогом.", $path),
                );
            }
            if (!is_readable($path) || !is_writable($path) || !is_executable($path) || @scandir($path) === false) {
                throw new \InvalidArgumentException(
                    sprintf("Каталог «%s» должен быть доступен для " . "чтения, записи и листинга.", $path),
                );
            }
        }
        $usedStrategies = [];
        foreach ($fileStrategies as &$strategy) {
            ...
            if (isset($usedStrategies[$strategy["code"]])) {
                throw new \InvalidArgumentException("Коды файловых стратегий должны быть уникальными.");
            }
    ...
    private function storedSettings(): array {
        ...
        $settings =
            ...
            is_array($data["settings.backups"] ?? null)
