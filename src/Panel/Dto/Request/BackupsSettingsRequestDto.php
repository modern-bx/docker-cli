    ...
    private static function validateStrategies(mixed $strategies): array {
        ...
        foreach ($strategies as $strategy) {
            if (
                ...
                !is_string($strategy["name"]) ||
                trim($strategy["name"]) === "" ||
                strlen($strategy["name"]) > 255 ||
                !is_string($strategy["code"]) ||
                !is_array($strategy["include"]) ||
                !array_is_list($strategy["include"]) ||
                !is_array($strategy["exclude"]) ||
                !array_is_list($strategy["exclude"]) ||
                !is_array($strategy["databaseInclude"]) ||
                !array_is_list($strategy["databaseInclude"]) ||
                !is_array($strategy["databaseExclude"]) ||
                !array_is_list($strategy["databaseExclude"])
            ) {
