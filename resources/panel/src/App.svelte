<script>
  ...
  import DeploymentListParameter from './components/DeploymentListParameter.svelte';
  ...
  async function openProjectCloneDialog(project) {
    ...
    try {
      ...
      projectCloneDialog = { project: project.name, to: '', location: location?.code || '', mysql: true, postgres: true, mirror: false, mirrorTree: true, mirrorDb: false, dedicated: dedicatedMysql || dedicatedPostgres, dedicatedMysql, dedicatedPostgres, locationMysql: databaseLocation, locationPostgres: databaseLocation };
    } catch (cause) {
  ...
  async function submitProjectClone() {
    ...
    try {
      ...
      await cloneProject(api, dialog.project, {
        ...
        dbms,
        mirror: dialog.mirror ? [dialog.mirrorTree && 'tree', dialog.mirrorDb && 'db'].filter(Boolean) : null,
    ...
    <Dialog.Content class="login-error-dialog project-clone-dialog card preset-filled-surface-100-900 shadow-2xl">
      ...
      {#if projectCloneDialog}
        <form class="project-add-form project-clone-form" onsubmit={(event) => { event.preventDefault(); submitProjectClone(); }}>
          ...
          <fieldset class="project-clone-dbms"><legend class="label-text"><span class="project-clone-help-heading">Быстрое клонирование<Tooltip positioning={{ placement: 'right' }}><Tooltip.Trigger class="security-help" type="button" aria-label="О быстром клонировании"><CircleHelp size={16} aria-hidden="true" /></Tooltip.Trigger><Tooltip.Positioner><Tooltip.Content class="security-tooltip card preset-filled-surface-900-100 shadow-xl">На одном разделе Btrfs файловое дерево и данные выделенных инстансов БД клонируются через CoW. Для БД исходный и целевой инстансы должны быть выделенными.</Tooltip.Content></Tooltip.Positioner></Tooltip></span></legend><label class="project-deployment-checkbox project-dedicated-toggle"><input class="checkbox" type="checkbox" bind:checked={projectCloneDialog.mirror} /><span>Использовать быстрые механизмы файловой системы</span></label>{#if projectCloneDialog.mirror}<div class="project-deployment-checkboxes"><label class="project-deployment-checkbox"><input class="checkbox" type="checkbox" bind:checked={projectCloneDialog.mirrorTree} /><span>Файловое дерево</span></label><label class="project-deployment-checkbox"><input class="checkbox" type="checkbox" bind:checked={projectCloneDialog.mirrorDb} disabled={!projectCloneDialog.dedicatedMysql && !projectCloneDialog.dedicatedPostgres} /><span>Базы данных</span></label></div>{/if}</fieldset>
            ...
            <div class="project-deployment-arguments">
              {#each Object.entries(selectedDeploymentScript.parameters) as [name, parameter], parameterIndex}
                ...
                {:else if parameter.type === 'list'}
                  <div class="label project-parameter-field"><span class="project-parameter-heading"><label class="label-text" for={`deployment-parameter-${parameterIndex}`}>{parameter.name || name}{parameter.required ? ' *' : ''}</label>{#if parameter.description}<Tooltip><Tooltip.Trigger class="security-help project-parameter-help" type="button" aria-label={`Описание параметра ${parameter.name || name}`}><CircleHelp size={15} aria-hidden="true" /></Tooltip.Trigger><Tooltip.Positioner><Tooltip.Content class="security-tooltip card preset-filled-surface-900-100 shadow-xl">{parameter.description}</Tooltip.Content></Tooltip.Positioner></Tooltip>{/if}</span><DeploymentListParameter id={`deployment-parameter-${parameterIndex}`} items={parameter.items || []} required={parameter.required} value={projectAddDialog.deploymentArguments[name]} onChange={(value) => setDeploymentArgument(name, value)} /></div>
                {:else}
