...
final class MysqlRecovery
{
    ...
    public function recover(
        ...
        $output->writeln(
            sprintf("<info>Обнаружен MySQL %s. Создаётся " . "изолированная копия данных.</info>", $version),
        );
        try {
            ...
            $this->mustRun(
                [
                    ...
                    "mysql:" . $version,
                    "-ec",
                    "cp -a /source/. /target/ && chown -R mysql:mysql /target",
                ],
                $output,
            );
            ...
            $this->mustRun(
                [
                    ...
                    "--skip-column-names",
                    "-e",
                    "SET GLOBAL sql_mode = TRIM(BOTH ',' FROM REPLACE(CONCAT(',', @@GLOBAL.sql_mode, " .
                    "','), ',NO_AUTO_CREATE_USER,', ','))",
                ],
                $output,
                true,
            ...
            [, $databaseOutput] = $this->mustRun(
                ["docker", "exec", $container, "mysql", "--batch", "--skip-column-names", "-e", "SHOW DATABASES"],
                $output,
                true,
            );
            $available = array_values(
                array_diff(
                    array_filter(array_map("trim", preg_split("/\R/", $databaseOutput) ?: [])),
            ...
            foreach ($databases as $database) {
                ...
                $this->mustRun(
                    [
                        ...
                        "--entrypoint",
                        "sh",
                        "mydumper/mydumper:v1.0.3-1",
                        "-ec",
                        'mydumper --host="$1" --user=root --database="$2" --outputdir=/dump --threads=4 ' .
                        '&& chown -R "$3:$4" /dump',
                        "sh",
                        $container,
                        $database,
    ...
    public function detectVersion(string $source): string
    {
        ...
        throw new \InvalidArgumentException(
            "Не удалось определить поддерживаемую " . "версию MySQL (5.5–5.7 или 8.0) по содержимому data.",
        );
    }
