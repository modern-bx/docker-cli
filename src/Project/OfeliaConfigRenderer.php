    ...
    public function render(): string {
        ...
        if (!is_dir(dirname($file)) && !mkdir(dirname($file), 0755, true) && !is_dir(dirname($file))) {
            throw new \RuntimeException(
                sprintf('Не удалось создать директорию конфигурации Ofelia "%s".', dirname($file)),
            );
        }
        ...
        foreach ($projects->registeredProjectNames() as $projectName) {
            ...
            $version = is_string($project["language_version"] ?? null)
                ? $project["language_version"]
                : PhpLanguageVersion::default($compose);
                ...
                $directory =
                    $workingDirectory === ""
                        ...
                        : (str_starts_with($workingDirectory, "/")
                            ? $workingDirectory
                            : join_path($root, $workingDirectory));
        ...
        if (file_put_contents($file, implode("\n", $lines), LOCK_EX) === false) {
            throw new \RuntimeException("Не удалось записать конфигурацию Ofelia.");
        }
