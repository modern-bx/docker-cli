...
Выделенные инстансы MySQL и PostgreSQL по умолчанию наследуются от исходного
проекта и создаются заново для клона. Набор можно переопределить через
`--dedicated-db=mysql,postgres`, а каталоги данных — через `--location-mysql` и
`--location-postgres`.
