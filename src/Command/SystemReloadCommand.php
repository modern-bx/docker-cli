...
final class SystemReloadCommand extends AbstractCommand
{
    ...
    public function __construct(?TranslatorInterface $translator = null)
    {
        ...
        $this->addOption(
            ...
            "Сервисы для reload, разделённые запятыми. По " . "умолчанию: все поддерживаемые.",
        );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($services as $service) {
            ...
            if (!is_resource($process)) {
                $this->writeMessage(
                    $output,
                    sprintf('<error>Не удалось запустить reload сервиса "%s".</error>', $service),
                );
            ...
            if ($code !== Command::SUCCESS) {
                $this->writeMessage(
                    $output,
                    sprintf('<error>Reload сервиса "%s" завершился с кодом %d.</error>', $service, $code),
                );
