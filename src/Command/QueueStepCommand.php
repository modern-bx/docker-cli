    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        try {
            ...
            if ($pending === null) {
                $this->writeMessage(
                    $output,
                    sprintf('<info>В очереди "%s" нет элементов для обработки.</info>', $queue),
                );
                return Command::SUCCESS;
            }
            ...
            foreach ($item["queue-item"]["tasks"] as $index => $task) {
                $repository->trace(
                    ...
                    $queue,
                    $item,
                    sprintf("Запуск задачи %s.", $task["code"]),
                    $task["code"],
                    $task["project"] ?? null,
                );
                ...
                $runner = new TaskRunCommand(
                    $tasks,
                    notifications: new NotificationRepository($repository->configDirectory()),
                );
                ...
                $repository->trace(
                    ...
                    $queue,
                    $item,
                    sprintf("Задача %s завершилась с кодом %d.", $task["code"], $exitCode),
                    $task["code"],
                    $task["project"] ?? null,
                    $exitCode,
                );
                if ($exitCode !== Command::SUCCESS) {
                    ...
                    $repository->trace(
                        ...
                        $queue,
                        $item,
                        "Элемент перемещен в 40-failure; последующие " . "задачи пропущены.",
                        level: "warning",
                    );
                    return $exitCode;
                }
            ...
            if (isset($active, $item) && is_string($active) && is_file($active) && is_array($item)) {
                try {
                    $repository->trace(
                        ...
                        $queue,
                        $item,
                        "Ошибка обработки: " . $exception->getMessage(),
                        level: "error",
                    );
                    $active = $repository->move($active, $queue, "50-error");
                    $repository->trace($active, $queue, $item, "Элемент перемещен в 50-error.", level: "error");
