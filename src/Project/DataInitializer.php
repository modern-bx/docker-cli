...
final class DataInitializer
{
    ...
    public function initialize(
        ...
        [
            ...
            $postgresGrantSql,
        ] = $this->postgresSql($projectName, $postgresPassword);
        return $this->run(
            array_merge($compose->dockerComposeCommand("exec"), [
                "-T",
                ...
                database_exists="$(
                    psql -v ON_ERROR_STOP=1 -At -U "$root_user" -d postgres -c "$database_exists_sql"
                )"
    ...
    public function clonePostgres(
        string $sourceDatabase,
        string $targetDatabase,
        string $sourceUser,
        string $targetUser,
        OutputInterface $output,
        string $sourceHostname = "postgres",
        string $targetHostname = "postgres",
    ): int {
        $compose = $this->compose ?? new SystemCompose();
        $compose->assertInitialized();
        ...
        return $this->run(
            array_merge($compose->dockerComposeCommand("exec"), [
                ...
                "WHERE datname = '{$targetLiteral}' AND pid <> pg_backend_pid();",
                "DROP DATABASE IF EXISTS {$targetIdentifier};",
                "ALTER DATABASE {$sourceIdentifier} WITH ALLOW_CONNECTIONS false;",
                "SELECT pg_terminate_backend(pid) FROM pg_stat_activity " .
                "WHERE datname = '{$sourceLiteral}' AND pid <> pg_backend_pid();",
                "CREATE DATABASE {$targetIdentifier} WITH TEMPLATE {$sourceIdentifier} STRATEGY = FILE_COPY;",
                "ALTER DATABASE {$targetIdentifier} OWNER TO {$targetUserIdentifier}; " .
                "GRANT ALL PRIVILEGES ON DATABASE {$targetIdentifier} TO {$targetUserIdentifier};",
                $targetDatabase,
                "REASSIGN OWNED BY {$sourceUserIdentifier} TO {$targetUserIdentifier}; " .
                "ALTER DATABASE {$sourceIdentifier} OWNER TO {$sourceUserIdentifier};",
            ]),
            $compose,
            $output,
        );
    ...
    private function mysqlSql(string $name, string $password, bool $rebuild): string
    {
        ...
        return $drop .
            " CREATE DATABASE IF NOT EXISTS `{$identifier}` CHARACTER SET utf8mb4 " .
            "COLLATE utf8mb4_unicode_ci; CREATE USER IF NOT EXISTS '{$user}'@'%' " .
            "IDENTIFIED BY '{$password}'; ALTER USER '{$user}'@'%' IDENTIFIED BY '{$password}'; " .
    ...
    private function postgresSql(string $name, string $password): array
    {
        ...
        return [
            "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '{$literalName}') " .
            "THEN EXECUTE format('CREATE ROLE %I LOGIN PASSWORD %L', '{$literalName}', '{$literalPassword}'); " .
            "ELSE EXECUTE format('ALTER ROLE %I LOGIN PASSWORD %L', '{$literalName}', '{$literalPassword}'); " .
            "END IF; END $$;",
            "DROP ROLE IF EXISTS {$quotedIdentifier};",
            "SELECT pg_terminate_backend(pid) FROM pg_stat_activity " .
            "WHERE datname = '{$literalName}' AND pid <> pg_backend_pid();",
    ...
    private function applyMysqlFile(
        SystemCompose $compose,
        string $database,
        string $file,
        OutputInterface $output,
    ...
    private function applyPostgresFile(
        SystemCompose $compose,
        string $database,
        string $file,
        OutputInterface $output,
        ...
        return $this->runWithInput(
            array_merge($compose->dockerComposeCommand("exec"), [
                ...
                "sh",
                "-ec",
                'export PGPASSWORD="${POSTGRES_PASSWORD:?}"; psql -v ON_ERROR_STOP=1 -U ' .
                '"${POSTGRES_USER:-system}" -d "$1"',
                "sh",
                $database,
            ]),
            $compose,
    ...
    private function runWithOutput(
        array $command,
        SystemCompose $compose,
        string $outputFile,
        OutputInterface $output,
