...
final class ProjectUpCommand extends AbstractCommand {
    ...
    public function __construct(
        ...
        $this->addArgument(
            ...
            "Имя проекта. По умолчанию используется " . "нормализованное имя директории проекта.",
        );
        ...
        $this->addOption(
            ...
            "Зарегистрировать проект, даже если " . "фреймворк не удалось определить.",
        );
        ...
        $this->addOption(
            ...
            null,
            InputOption::VALUE_REQUIRED,
            "Выделенные СУБД через запятую: mysql, postgres.",
        );
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        $dedicatedDatabases = array_values(
            array_unique(
                array_filter(array_map("trim", explode(",", (string) ($input->getOption("dedicated-db") ?? "")))),
            ),
        );
        ...
        foreach (["mysql", "postgres"] as $driver) {
            ...
            if (!in_array($driver, $dedicatedDatabases, true)) {
                $this->writeMessage(
                    $output,
                    sprintf(
                        "<error>Опцию --location-%s можно использовать " . "только вместе с --dedicated-db=%s.</error>",
                        $driver,
                        $driver,
                    ),
                );
        ...
        if (!$this->isValidProjectName($projectName)) {
            $this->writeMessage(
                $output,
                sprintf(
                    '<error>Имя проекта "%s" не соответствует ' .
                        "конвенции: используйте строчные латинские " .
                        "буквы, цифры и дефисы; имя должно " .
                        "начинаться и заканчиваться буквой или " .
                        "цифрой.</error>",
                    $projectName,
                ),
            );
        ...
        if (!is_dir($projectsDirectory) && !mkdir($projectsDirectory, 0775, true) && !is_dir($projectsDirectory)) {
            $this->writeMessage(
                $output,
                sprintf('<error>Не удалось создать директорию "%s".</error>', $projectsDirectory),
            );
        ...
        if (!mkdir($projectDirectory, 0775) && !is_dir($projectDirectory)) {
            $this->writeMessage(
                $output,
                sprintf('<error>Не удалось создать директорию проекта "%s".</error>', $projectDirectory),
            );
        ...
        $documentRoot =
            $framework?->getDocumentRoot() ??
            (in_array($frameworkCode, ["symfony", "laravel"], true) ? join_path($projectRoot, "public") : $projectRoot);

        if ($frameworkCode === null) {
            $this->writeMessage(
                $output,
                "<comment>Фреймворк проекта не определен; " .
                    "проект будет зарегистрирован с базовой " .
                    "веб-конфигурацией.</comment>",
            );
        }
        ...
        if (
            !is_dir($projectMetadataDirectory) &&
            !mkdir($projectMetadataDirectory, 0775, true) &&
            !is_dir($projectMetadataDirectory)
        ) {
            $this->writeMessage(
                $output,
                sprintf('<error>Не удалось создать директорию "%s".</error>', $projectMetadataDirectory),
            );
        ...
        if (
            !is_dir($projectDataDirectory) &&
            !mkdir($projectDataDirectory, 0775, true) &&
            !is_dir($projectDataDirectory)
        ) {
            $this->writeMessage(
                $output,
                sprintf('<error>Не удалось создать директорию "%s".</error>', $projectDataDirectory),
            );
        ...
        try {
            $dataInitCode = (new DataInitializer())->initialize(
                $projectName,
                $mysqlPassword,
                $postgresPassword,
                false,
    ...
    private function resolveProjectName(
        ...
        if ($directoryName === "") {
            $this->writeMessage(
                $output,
                sprintf(
                    "<comment>Не удалось сформировать имя проекта " .
                        'из имени директории "%s"; используется ' .
                        'сгенерированное имя "%s".</comment>',
