...
final class TreeArchiveManager
{
    ...
    public function dump(
        ...
        $projectRoot =
            realpath($projectRoot) ?: throw new \InvalidArgumentException("Корневая директория проекта не существует.");
    ...
    private function createTar(string $projectRoot, string $tar, ?string $manifest): void
    {
        ...
        if ($manifest !== null) {
            ...
            array_push($command, "--exclude=./.docker-cli", "--exclude=./.docker-cli-task-*", ".");
        }
        $this->run($command);
    }

    private function streamCompressedTar(
        string $projectRoot,
        string $archive,
        string $compressor,
        ?string $manifest,
        ...
        if ($manifest !== null) {
            array_push($tarCommand, "--null", "--verbatim-files-from", "--no-recursion", "-T", $manifest);
        } else {
            array_push($tarCommand, "--exclude=./.docker-cli", "--exclude=./.docker-cli-task-*", ".");
        }
        ...
        if (!$this->executable("tar")) {
            throw new \RuntimeException("Команда «tar» не найдена.");
        }
        if (!$this->executable($compressCommand[0])) {
            throw new \RuntimeException(sprintf("Команда «%s» не найдена.", $compressCommand[0]));
        }

        $target = fopen($archive, "wb");
        if (!is_resource($target)) {
            throw new \RuntimeException("Не удалось создать файл архива.");
        }
        ...
        if ($tarCode !== 0 || $compressCode !== 0) {
            throw new \RuntimeException(
                sprintf(
                    "Архивация завершилась с ошибкой (tar: %d, %s: %d).",
                    $tarCode,
                    $compressCommand[0],
                    $compressCode,
                ),
    ...
    public function selectedPaths(string $projectRoot, array $include, array $exclude): array
    {
        ...
        foreach ($iterator as $item) {
            ...
            if (
                $path === ".docker-cli" ||
                str_starts_with($path, ".docker-cli/") ||
                str_starts_with($path, ".docker-cli-task-")
            ) {
                continue;
            }
    ...
    private function matchesAny(string $path, array $patterns): bool
    {
        foreach ($patterns as $pattern) {
            $candidate = $path;
            while (true) {
                if ($this->globMatches($pattern, $candidate)) {
                    return true;
                }
    ...
    private function compress(string $tar, string $compressor): string
    {
        ...
        switch ($compressor) {
            case "zip":
                ...
                if (!unlink($tar)) {
                    throw new \RuntimeException("Не удалось удалить исходный tar-архив.");
                }
                break;
            default:
        ...
        if (!is_file($target)) {
            throw new \RuntimeException("Архиватор не создал ожидаемый файл.");
        }
        return $target;
    }
    ...
    private function run(array $command): void
    {
        ...
        if ($code !== 0) {
            throw new \RuntimeException(sprintf("Команда «%s» завершилась с кодом %d.", $command[0], $code));
        }
    }
