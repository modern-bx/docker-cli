...
final class PostgresDumpCommand extends AbstractCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!is_string($database) || $database === "") {
            $this->writeMessage(
                $output,
                sprintf('<error>В конфигурации проекта "%s" не задана ' . "база PostgreSQL.</error>", $project),
            );
            return Command::FAILURE;
        }
        ...
        try {
            $code = ($this->dumpLoader ?? new PostgresDumpLoader())->dump(
                $database,
                $path,
                $jobs,
                $output,
        ...
        if ($code === Command::SUCCESS) {
            ...
            CommandContext::fromEnvironment($this, $output)->addMessage(
                new Message(
                    sprintf(
                        'Бэкап PostgreSQL-базы "%s" проекта "%s" создан: "%s".',
                        $database,
                        $project,
                        basename($path),
                    ),
