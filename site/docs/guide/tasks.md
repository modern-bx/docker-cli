...
Содержимое `action` исполняется Bash без дополнительной интерпретации его вывода. Поэтому `echo "docker-cli ..."` только печатает строку. Для запуска команды её нужно указать в `action` напряму:

```yaml
action: |
  docker-cli project:wipe --project={{ project }}
```
