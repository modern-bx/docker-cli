...
final readonly class ProjectScheduleRequestDto implements RequestDto {
    ...
    public static function fromRequest(RequestData $request): static {
        ...
        if (!is_string($workingDirectory)) {
            throw new RequestValidationException("Некорректная рабочая папка.");
        }
        if (!is_bool($request->body["enabled"] ?? null)) {
            throw new RequestValidationException("Некорректный статус команды.");
        }

        $index = isset($request->route["index"])
            ? filter_var($request->route["index"], FILTER_VALIDATE_INT, ["options" => ["min_range" => 0]])
            : null;
        if ($index === false) {
            throw new RequestValidationException("Некорректный номер записи расписания.");
        }
        ...
        return new static(
            ...
            trim($request->body["schedule"]),
            trim($request->body["command"]),
            trim($workingDirectory),
        );
    }
}
