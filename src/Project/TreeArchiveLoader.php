...
final readonly class TreeArchiveLoader
{
    ...
    public function load(string $backupDirectory, string $projectRoot, bool $force, bool $wipe): void
    {
        ...
        try {
            ...
            if ($wipe) {
                ($this->manager ?? new TreeArchiveManager())->wipeProject($projectRoot);
            }
            if (!$force) {
                foreach ($entries as $entry) {
                    ...
                    if (
                        ...
                        throw new \InvalidArgumentException(
                            sprintf("Файл «%s» уже существует. Используйте --force " . "для перезаписи.", $relative),
                        );
                    }
                }
            }
    ...
    private function tarCommand(string $archive, string $archiveName, string $operation, array $suffix = []): array
    {
        if (str_ends_with($archiveName, ".tar.lz4")) {
            return [
                ...
                $operation .
                " - " .
                implode(" ", array_map("escapeshellarg", $suffix)),
            ];
        }
    ...
    private function assertSafeEntry(string $entry): void
    {
        ...
        if (
            ...
            $normalized === ".docker-cli" ||
            str_starts_with($normalized, ".docker-cli/")
        ) {
    ...
    private function run(array $command, bool $capture = false): string
    {
        ...
        if (!is_resource($process)) {
            throw new \RuntimeException("Не удалось запустить распаковку файлового бэкапа.");
        }
        ...
        if ($code !== 0) {
            throw new \RuntimeException(sprintf("Распаковка файлового бэкапа завершилась с кодом %d.", $code));
        }
