...
final class PostgresRecovery {
    ...
    public function recover(
        ...
        $output->writeln(
            sprintf("<info>Обнаружен PostgreSQL %s. Создаётся " . "изолированная копия данных.</info>", $version),
        );
        try {
            ...
            $this->mustRun(
                [
                    ...
                    "postgres:" . $version,
                    "-ec",
                    "cp -a /source/. /target/ && chown -R postgres:postgres /target",
                ],
                $output,
            );
            $this->mustRun(
            ...
            $this->mustRun(
                [
                    ...
                    "'host all all ::/0 trust' >/tmp/pg_hba.conf; exec gosu postgres postgres " .
                    "-D /var/lib/postgresql/data -c listen_addresses='*' -c hba_file=/tmp/pg_hba.conf",
                ],
                $output,
            );
            ...
            [, $databaseOutput] = $this->mustRun(
                [
                    ...
                    "--dbname=template1",
                    "--command",
                    "SELECT datname FROM pg_database WHERE datallowconn AND NOT datistemplate AND " .
                    "datname <> 'postgres' ORDER BY datname",
                ],
                $output,
                true,
            );
            ...
            foreach ($databases as $database) {
                ...
                $this->mustRun(
                    [
                        ...
                        "postgres:" . $version,
                        "-ec",
                        'database="$1"; jobs="$2"; uid="$3"; gid="$4"; pg_dump --host="$5" ' .
                        '--username="$6" --format=directory --jobs="$jobs" --file=/dump "$database"; ' .
                        'chown -R "$uid:$gid" /dump',
                        "sh",
                        $database,
                        "4",
                        (string) $this->uid(),
    ...
    public function detectVersion(string $source): string {
        if (
            ...
            !is_dir($source . "/base") ||
            !is_dir($source . "/global")
        ) {
            throw new \InvalidArgumentException(
                sprintf('Директория "%s" не похожа на каталог data PostgreSQL.', $source),
            );
        }
        $version = trim((string) @file_get_contents($source . "/PG_VERSION"));
        if (preg_match('/^[1-9]\d*(?:\.\d+)?$/', $version) !== 1) {
            throw new \InvalidArgumentException(
                sprintf('Файл PG_VERSION содержит неподдерживаемую версию: "%s".', $version),
            );
        }
        return $version;
    }
    ...
    private function prepareDestination(string $destination): void {
        ...
        if (!is_writable($destination)) {
            throw new \InvalidArgumentException(
                sprintf('Директория назначения "%s" недоступна для записи.', $destination),
            );
        }
    }
    ...
    private function mustRun(array $command, OutputInterface $output, bool $quiet = false): array {
        ...
        if ($result[0] !== 0) {
            throw new \RuntimeException(
                sprintf("Команда Docker завершилась с кодом %d:\n%s", $result[0], trim($result[1])),
            );
        }
        return $result;
    }
    ...
    private function run(array $command, bool $capture): array {
        ...
        if (!is_resource($process)) {
            throw new \RuntimeException("Не удалось запустить Docker.");
        }
