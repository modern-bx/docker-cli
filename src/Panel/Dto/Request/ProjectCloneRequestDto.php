...
final readonly class ProjectCloneRequestDto implements RequestDto
{
    ...
    public static function fromRequest(RequestData $request): static
    {
        ...
        if (
            ...
            !is_array($dbms) ||
            !array_is_list($dbms) ||
            ($mirror !== null && (!is_array($mirror) || !array_is_list($mirror))) ||
            ($dedicatedDatabases !== null && (!is_array($dedicatedDatabases) || !array_is_list($dedicatedDatabases))) ||
        ...
        if (
            ...
            throw new RequestValidationException(
                "Зеркалирование поддерживает только " . "файловое дерево и базы данных.",
            );
        }
        if (
            array_filter(
        ...
        if (
            ...
            throw new RequestValidationException(
                "Выделенные инстансы поддерживаются только " . "для MySQL и PostgreSQL.",
            );
        }
        return new static(
            $name,
            is_string($to) && trim($to) !== "" ? trim($to) : null,
            is_string($location) && trim($location) !== "" ? trim($location) : null,
            $skipDb,
            array_values(array_unique($dbms)),
            $mirror === null ? null : array_values($mirror),
            $dedicatedDatabases === null ? null : array_values($dedicatedDatabases),
            $locationMysql,
            $locationPostgres,
        );
    }
