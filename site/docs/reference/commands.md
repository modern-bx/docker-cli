...
останавливает выполнение цепочки и возвращается как код завершения команды. Примеры хуков хранятся в `resources/actions/hooks` и копируются только командой `config:init --examples`; их имена начинаются с точки, поэтому после копирования они остаются отключенными, пока пользователь не переименует их.
