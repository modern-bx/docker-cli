...
Запускает постоянный обработчик очереди: команда последовательно обрабатывает новые элементы и ожидает следующие. По умолчанию используется очередь `default`. Штатный вывод задач и ошибок элементов не дублируется в `stdout`/`stderr`, поскольку он сохраняется в элементе и общем журнале очереди; в консоль попадают только критические ошибки самого обработчика.
