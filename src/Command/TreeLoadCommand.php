...
final class TreeLoadCommand extends AbstractCommand {
    public function __construct(
        ...
        $this->addOption(
            ...
            "Очистить проект перед восстановлением, " . "сохранив только .docker-cli.",
        );
    }
        ...
        try {
            ...
            if (!is_array($metadata) || ($metadata["project"] ?? null) !== $project) {
                throw new \InvalidArgumentException(
                    "Бэкап создан для другого проекта или " . "содержит некорректные метаданные.",
                );
            }
            ...
            if (
                $strategyCode !== null &&
                (!is_array($strategyPaths) ||
                    !is_array($strategyPaths["include"] ?? null) ||
                    !is_array($strategyPaths["exclude"] ?? null))
            ) {
            ...
            if ($input->getOption("wipe") && $strategyCode !== null) {
                $this->warnStrategyDifference($output, $strategyCode, $strategyPaths);
            }
    ...
    private function warnStrategyDifference(OutputInterface $output, string $code, array $savedPaths): void {
        ...
        if (
            $current === null ||
            $current["include"] !== $savedPaths["include"] ||
            $current["exclude"] !== $savedPaths["exclude"]
        ) {
            $this->writeMessage(
                $output,
                sprintf(
                    "<comment>Сохранённые пути стратегии «%s» " . "отличаются от текущих настроек.</comment>",
                    $code,
                ),
            );
        }
