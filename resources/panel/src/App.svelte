        ...
        {#if selectedDeploymentScript}
          ...
          {:else}
            <div class="project-deployment-arguments">
              {#each Object.entries(selectedDeploymentScript.parameters) as [name, parameter]}
                {#if parameter.type === 'boolean'}
                  <label class="project-deployment-checkbox"><input class="checkbox" type="checkbox" checked={projectAddDialog.deploymentArguments[name] === true} onchange={(event) => setDeploymentArgument(name, event.currentTarget.checked)} /><span class="project-parameter-title">{parameter.name || name}{#if parameter.description}<Tooltip><Tooltip.Trigger class="security-help project-parameter-help" type="button" aria-label={`Описание параметра ${parameter.name || name}`} onclick={(event) => event.preventDefault()}><CircleHelp size={15} aria-hidden="true" /></Tooltip.Trigger><Tooltip.Positioner><Tooltip.Content class="security-tooltip card preset-filled-surface-900-100 shadow-xl">{parameter.description}</Tooltip.Content></Tooltip.Positioner></Tooltip>{/if}</span></label>
                {:else if parameter.type === 'list'}
                  <label class="label"><span class="label-text project-parameter-title">{parameter.name || name}{parameter.required ? ' *' : ''}{#if parameter.description}<Tooltip><Tooltip.Trigger class="security-help project-parameter-help" type="button" aria-label={`Описание параметра ${parameter.name || name}`} onclick={(event) => event.preventDefault()}><CircleHelp size={15} aria-hidden="true" /></Tooltip.Trigger><Tooltip.Positioner><Tooltip.Content class="security-tooltip card preset-filled-surface-900-100 shadow-xl">{parameter.description}</Tooltip.Content></Tooltip.Positioner></Tooltip>{/if}</span><select class="select" required={parameter.required} value={projectAddDialog.deploymentArguments[name]} onchange={(event) => setDeploymentArgument(name, event.currentTarget.value)}><option value="">Не выбрано</option>{#each parameter.items || [] as item}<option value={item.value}>{item.name || item.value}</option>{/each}</select></label>
                {:else}
                  <label class="label"><span class="label-text project-parameter-title">{parameter.name || name}{parameter.required ? ' *' : ''}{#if parameter.description}<Tooltip><Tooltip.Trigger class="security-help project-parameter-help" type="button" aria-label={`Описание параметра ${parameter.name || name}`} onclick={(event) => event.preventDefault()}><CircleHelp size={15} aria-hidden="true" /></Tooltip.Trigger><Tooltip.Positioner><Tooltip.Content class="security-tooltip card preset-filled-surface-900-100 shadow-xl">{parameter.description}</Tooltip.Content></Tooltip.Positioner></Tooltip>{/if}</span><input class="input" type={parameter.type === 'integer' ? 'number' : 'text'} required={parameter.required} min={parameter.min} max={parameter.max} value={projectAddDialog.deploymentArguments[name]} oninput={(event) => setDeploymentArgument(name, parameter.type === 'integer' && event.currentTarget.value !== '' ? event.currentTarget.valueAsNumber : event.currentTarget.value)} /></label>
                {/if}
              {/each}
            </div>
          {/if}
