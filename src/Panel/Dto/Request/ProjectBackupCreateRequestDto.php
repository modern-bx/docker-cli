...
final readonly class ProjectBackupCreateRequestDto implements RequestDto
{
    ...
    public static function fromRequest(RequestData $request): static
    {
        ...
        if (
            !is_string($strategy) ||
            ($strategy !== "" && preg_match('/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/', $strategy) !== 1) ||
            !is_string($compress) ||
            !in_array($compress, ["", "gzip", "bzip2", "xz", "zstd", "lz4", "zip"], true)
        ) {
            throw new RequestValidationException("Некорректные параметры файлового бэкапа.");
        }
        if (
            !is_string($chunkSize) ||
            !is_string($chunkCount) ||
            ...
            throw new RequestValidationException(
                "Укажите либо корректный размер тома, либо " . "количество томов не меньше двух.",
            );
        }
